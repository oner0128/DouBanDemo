...
public class PutMoviesToSQLite {
    ...
    public static void getComingsoonMoviesFromJson(Activity activity , MoviesBean moviesBean) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_TITLE, title);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_DOUBAN_ID, douban_id);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_ALT, alt);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_RELEASE_DATE, release_date);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_GENERE_IDS, genre_ids);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_IMAGE_POSTER, image_poster);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_VOTE_COUNT, vote_count);
            weatherValues.put(MoviesContract.ComingsoonMoviesEntry.COLUMN_VOTE_AVERAGE, vote_average);
            valuesVector.add(weatherValues);
        }
        int isInsert = 0;
        if (valuesVector.size() > 0) {
            ContentValues[] contentValues = new ContentValues[valuesVector.size()];
            valuesVector.toArray(contentValues);
            isInsert = activity.getContentResolver().bulkInsert(MoviesContract.ComingsoonMoviesEntry.CONTENT_URI, contentValues);
        }
        if (isInsert != 0)
            Log.d(LOG_TAG, LOG_TAG + " complete." + isInsert + " inserted (comingsoon)");
    }
}
